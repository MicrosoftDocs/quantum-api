...
| Name | Summary |
|------|---------|
|[ApplyFunctionWithLookupTable](xref:Microsoft.Quantum.Arithmetic.ApplyFunctionWithLookupTable) |This function creates a select-swap lookup table operator for the function that you want to approximate, as well as the parameters required to make the two FixedPoint registers that need to be used as inputs to the operator. This is so that it is in similar to the other typical Q# arithmetic function (a larger discussion can be had as to whether that can be changed). The circuit for the operator can be found in Fig. 1c in arXiv:1812.00954. |
